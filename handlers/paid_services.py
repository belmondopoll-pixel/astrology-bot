...
@router.message(F.text == "💑 Совместимость (55 звезд)")
    ...
    try:
        ...
        if await payment_service.process_payment(callback.from_user.id, "compatibility"):
            ...
            await callback.message.edit_text(
                ...
                "Для использования этой услуги требуется 55 Telegram Stars.\n"
    ...
    try:
        ...
        if await payment_service.process_payment(callback.from_user.id, "tarot_reading"):
            ...
            try:
                ...
                await callback.message.edit_text(cards_display, parse_mode="HTML")
                ...
                spread_description = await create_tarot_prompt(spread_type, cards, positions)
...
@router.message(F.text == "🌌 Натальная карта (999 звезд)")
...
async def process_birth_place(message: Message, state: FSMContext):
    ...
    if await payment_service.process_payment(message.from_user.id, "natal_chart"):
        ...
        try:
            ...
            await generating_msg.edit_text(first_part, parse_mode="HTML")
            ...
            await generating_msg.edit_text(error_text, parse_mode="HTML")
    else:
        await message.answer(
            "❌ <b>Недостаточно средств</b>\n\n"
            "Для использования этой услуги требуется 999 Telegram Stars.\n"
            "Пополните баланс и попробуйте снова."
        )
